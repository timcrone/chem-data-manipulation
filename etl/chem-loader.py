            ...
            _return += [
                {
                    ...
                    'logp': chunks[8],
                    'reactive': chunks[10],
                    'features': feature_map(chunks[11])
...
class ChemLoader:
    ...
    def __init__(self, tree):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=7) as executor:
            futures = {executor.submit(load_chem, file_name) for file_name in _tranche_files}
            for future in concurrent.futures.as_completed(futures):
                _tranche_data = future.result()
                self.tranche[_tranche_data[0]] = _tranche_data[1]
                logging.info("Completed %s", _tranche_data[0])
